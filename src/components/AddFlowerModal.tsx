...
import { type ScoreItem, getAddItems } from '../lib/data-manager';
...
export function AddFlowerModal({ 
  ...
  const addItems = getAddItems();

  const handleReasonClick = (item: ScoreItem) => {
    // 防止重复点击
    if (processingRef.current) {
      console.warn('[AddModal] 正在处理中，忽略点击');
      return;
    }
    ...
    console.log(`[AddModal] 开始处理: ${item.name}`);
      ...
      <div className="relative w-full max-w-[390px] max-h-[90vh]">
        <div className="bg-white rounded-3xl overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div className="flex justify-between items-center px-6 py-5 border-b border-gray-200">
            <h2 className="text-gray-900 text-2xl font-bold">赠送红花</h2>
            <button
              onClick={onClose}
